# Mobile Auth Test Project

Next.js + iOS + Android 프로젝트로 모바일 앱에서 Transcodes WebAuthn/Passkey 인증을 테스트하는 프로젝트입니다.

## 기술 스택

- **Next.js**: 15.3.4
- **React**: 19.1.0
- **TypeScript**: 5.8.3
- **iOS**: Swift (WKWebView)
- **Android**: Kotlin (WebView)
- **인증**: Transcodes SDK

## 개발 환경

이 프로젝트는 다음 개발 환경에서 작성 및 테스트되었습니다:

- **Xcode**: 16.2.0
- **Android Studio**: Ladybug Feature Drop | 2024.2.2 (Build #AI-242.23726.103.2422.12816248)
- **Runtime**: OpenJDK 64-Bit Server VM 21.0.4+ (aarch64)

## 프로젝트 구조

```
transcode-native-app-auth/
├── nextjs-app/          # Next.js 웹 앱 (WebView에서 로드)
│   ├── app/
│   │   ├── auth/
│   │   │   └── mobile/  # 인증 페이지
│   │   ├── layout.tsx   # SDK 스크립트 로드
│   │   └── page.tsx      # 홈 페이지
│   └── package.json
│
├── ios-app/             # iOS 네이티브 앱
│   └── MobileAuthTest/
│       └── MobileAuthTest/
│           ├── AuthWebViewController.swift  # WebView 컨트롤러
│           ├── MainViewController.swift     # 메인 화면
│           ├── KeychainHelper.swift          # Keychain 관리
│           ├── AppDelegate.swift
│           ├── SceneDelegate.swift
│           └── Info.plist                   # 네트워크 설정 포함
│
└── android-app/         # Android 네이티브 앱
    └── app/
        └── src/
            └── main/
                ├── java/com/transcodes/mobileauthtest/
                │   ├── MainActivity.kt           # 메인 화면
                │   ├── AuthWebViewActivity.kt    # WebView 컨트롤러
                │   └── TokenHelper.kt            # EncryptedSharedPreferences 관리
                ├── res/
                │   ├── layout/
                │   │   └── activity_main.xml
                │   └── values/
                │       └── strings.xml
                └── AndroidManifest.xml           # 네트워크 설정 포함
```

## 아키텍처

```
┌─────────────────────────────────────────┐  ┌─────────────────────────────────────────┐
│         iOS Native App                  │  │      Android Native App                 │
│  ┌───────────────────────────────────┐  │  │  ┌───────────────────────────────────┐  │
│  │  MainViewController               │  │  │  │  MainActivity                      │  │
│  │  - Login/Logout Button           │  │  │  │  - Login/Logout Button           │  │
│  │  - Status Display                 │  │  │  │  - Status Display               │  │
│  │  - Token Display                  │  │  │  │  - Token Display                  │  │
│  └──────────────┬────────────────────┘  │  │  └──────────────┬────────────────────┘  │
│                 │                        │  │                 │                        │
│                 ▼                        │  │                 ▼                        │
│  ┌───────────────────────────────────┐  │  │  ┌───────────────────────────────────┐  │
│  │  AuthWebViewController            │  │  │  │  AuthWebViewActivity            │  │
│  │  ┌─────────────────────────────┐  │  │  │  │  ┌─────────────────────────────┐  │
│  │  │  WKWebView                  │  │  │  │  │  │  WebView                    │  │
│  │  │  ┌───────────────────────┐  │  │  │  │  │  │  ┌───────────────────────┐  │
│  │  │  │  Next.js Auth Page    │  │  │  │  │  │  │  │  Next.js Auth Page    │  │
│  │  │  │  /auth/mobile         │  │  │  │  │  │  │  │  /auth/mobile         │  │
│  │  │  └───────────────────────┘  │  │  │  │  │  │  └───────────────────────┘  │
│  │  └─────────────────────────────┘  │  │  │  │  └─────────────────────────────┘  │
│  │  JS Bridge (WKScriptMessageHandler)│ │  │  │  JS Bridge (@JavascriptInterface)│
│  │  Console Logger                    │  │  │  │  Console Logger                  │
│  └───────────────────────────────────┘  │  │  └───────────────────────────────────┘
│                 │                        │  │                 │                        │
│                 ▼                        │  │                 ▼                        │
│  ┌───────────────────────────────────┐  │  │  ┌───────────────────────────────────┐  │
│  │  KeychainHelper                   │  │  │  │  TokenHelper                    │  │
│  │  - Save Token                     │  │  │  │  - Save Token                   │  │
│  │  - Get Token                      │  │  │  │  - Get Token                    │  │
│  │  - Delete Token                   │  │  │  │  - Delete Token                 │  │
│  └───────────────────────────────────┘  │  │  └───────────────────────────────────┘  │
└─────────────────────────────────────────┘  └─────────────────────────────────────────┘
         │                                              │
         └──────────────────┬───────────────────────────┘
                           ▼
┌─────────────────────────────────────────┐
│      Next.js Server (localhost:3001)     │
│  ┌───────────────────────────────────┐  │
│  │  app/layout.tsx                   │  │
│  │  - SDK Script Injection           │  │
│  └───────────────────────────────────┘  │
│  ┌───────────────────────────────────┐  │
│  │  app/auth/mobile/page.tsx        │  │
│  │  - Auth Flow                      │  │
│  │  - Token Retrieval                │  │
│  └───────────────────────────────────┘  │
└─────────────────────────────────────────┘
```

## 설정 방법

### 1. Next.js 앱 설정

#### 로컬 개발 환경 (참고용)

로컬에서 Next.js 앱을 실행하려면:

```bash
cd nextjs-app
npm install
```

`.env.local` 파일 생성:

```bash
touch .env.local
```

`.env.local` 파일을 열어서 Transcodes Project ID와 API Base URL을 설정:

```env
NEXT_PUBLIC_TRANSCODES_PROJECT_ID=proj_your_project_id_here
```

개발 서버 실행:

```bash
npm run dev
```

서버가 `http://localhost:3001`에서 실행됩니다.

**참고**: SDK는 `app/layout.tsx`에서 자동으로 로드됩니다. Project ID가 설정되면 `https://d2xt92e3v27lcm.cloudfront.net/{projectId}/webworker.js` 스크립트가 페이지에 주입됩니다.

#### 배포 환경 (실제 테스트)

**중요**: iOS와 Android 네이티브 앱에서는 **배포된 환경(Vercel)에서 테스트**해야 합니다.

- 로컬 개발 환경(`localhost:3001`)은 참고용이며, 실제 모바일 앱 테스트에는 사용하지 않습니다.
- iOS와 Android 앱은 모두 `https://transcode-native-app-auth.vercel.app`를 기본 API URL로 사용하도록 설정되어 있습니다.
- 로컬 개발 서버는 브라우저에서 직접 테스트하거나 개발 중 디버깅 목적으로만 사용하세요.

### 2. iOS 앱 설정

1. Xcode에서 `ios-app/MobileAuthTest.xcodeproj` 열기
2. **Signing & Capabilities** 탭에서:
   - Team 선택 (개인 개발자 계정 또는 팀)
   - Bundle Identifier 확인: `com.transcodes.mobileauthtest`
3. `Info.plist` 파일에서 `API_URL` 설정:
   - Xcode에서 `ios-app/MobileAuthTest/Info.plist` 파일 열기
   - `API_URL` 키 추가 (또는 기존 값 수정)
   ```xml
   <key>API_URL</key>
   <string>https://transcode-native-app-auth.vercel.app</string>
   ```
   
   **참고**: `AuthWebViewController.swift`는 `Info.plist`의 `API_URL`을 사용하여 환경변수에서 URL을 가져옵니다. 하드코딩된 URL이 없으며, `Info.plist`에서만 관리됩니다.

### 3. Android 앱 설정

1. Android Studio에서 `android-app` 폴더를 프로젝트로 열기
2. **Android SDK 경로 설정** (처음 열 때 다이얼로그가 나타날 수 있음):
   - macOS 기본 경로: `~/Library/Android/sdk` 또는 `/Users/[사용자명]/Library/Android/sdk`
   - 다이얼로그에서 "Select Android SDK:" 필드에 경로 입력
   - 폴더 아이콘을 클릭하여 파일 브라우저로 선택 가능
3. **Gradle Sync** 실행 (프로젝트가 열리면 자동으로 실행되거나 상단의 "Sync Now" 클릭)
4. `android-app/gradle.properties` 파일에서 `API_URL` 설정:
   ```properties
   # API Base URL
   API_URL=https://transcode-native-app-auth.vercel.app
   ```
   
   **참고**: `AuthWebViewActivity.kt`는 `BuildConfig.API_URL`을 사용하여 환경변수에서 URL을 가져옵니다. 하드코딩된 URL이 없으며, `gradle.properties`에서만 관리됩니다.

### 4. 네트워크 설정

#### iOS

1. **Info.plist**에 이미 `NSAppTransportSecurity` 설정이 포함되어 있습니다.
2. **환경변수 관리**: `Info.plist`의 `API_URL`을 변경한 후 앱을 다시 빌드해야 변경사항이 적용됩니다.

#### Android

1. **AndroidManifest.xml**에 `android:usesCleartextTraffic="true"` 설정이 포함되어 있습니다.
2. **환경변수 관리**: `gradle.properties`의 `API_URL`을 변경한 후 **Gradle Sync**를 다시 실행해야 변경사항이 적용됩니다.

## 주요 기능 상세

### 인증 플로우 (`/auth/mobile`)

1. SDK 자동 로드 및 초기화
2. `openAuthLoginModal()` 자동 호출
3. Passkey 인증 진행
4. 토큰 획득 (`getAccessToken()` API 사용)
5. Native 앱으로 토큰 전달
6. Keychain에 토큰 저장

## 사용 방법

### 1. Next.js 서버 실행

```bash
cd nextjs-app
npm run dev
```

### 2. iOS 앱 실행

1. Xcode에서 프로젝트 열기
2. 시뮬레이터 또는 실제 기기 선택
3. Run 버튼 클릭 (⌘R)

### 3. Android 앱 실행

1. Android Studio에서 프로젝트 열기
2. 에뮬레이터 또는 실제 기기 선택
3. Run 버튼 클릭 (▶️)

### 4. 테스트 플로우

1. 앱 실행 후 "Login with Passkey" 버튼 클릭
2. WebView가 열리고 Next.js 인증 페이지 (`/auth/mobile`) 로드
3. SDK가 자동으로 초기화되고 로그인 모달 표시
4. Passkey 인증 완료
5. 토큰이 Keychain에 저장되고 메인 화면으로 돌아감
6. 상태가 "✅ Authenticated"로 변경되고 토큰이 표시됨
7. 로그아웃하려면 "Logout" 버튼 클릭 (토큰이 Keychain에서 삭제됨)

## JS Bridge 통신 프로토콜

### WebView → Native (iOS/Android)

```typescript
// 인증 시작
sendToNative('AUTH_STARTED', {});

// 인증 성공
sendToNative('AUTH_SUCCESS', {
  token: string,
  user: {
    id: string,
    email?: string,
    name?: string
  }
});

// 인증 취소
sendToNative('AUTH_CANCELLED', {
  error?: string
});

// 인증 실패
sendToNative('AUTH_ERROR', {
  message: string
});
```

### Bridge 구현

- **iOS**: `window.webkit.messageHandlers.nativeBridge.postMessage()`
- **Android**: `window.AndroidBridge.postMessage()`

### Console Logging

**iOS**: WKWebView에서 JavaScript 콘솔 로그를 자동으로 캡처하여 Xcode 콘솔에 출력합니다.

**Android**: WebView의 `onConsoleMessage`를 통해 JavaScript 콘솔 로그를 Logcat에 출력합니다.

로그 레벨 매핑:
- `console.log` → `📝 [JS Console]`
- `console.error` → `❌ [JS Console]`
- `console.warn` → `⚠️ [JS Console]`
- `console.info` → `ℹ️ [JS Console]`
- `console.debug` → `🔍 [JS Console]`

## 디버깅

### Next.js 콘솔

브라우저 개발자 도구에서 확인:
- SDK 로드 상태 (`window.transcodes` 확인)
- 인증 플로우 (`[Page]` 접두사 로그)
- 에러 메시지

### iOS 콘솔

Xcode 콘솔에서 확인:
- `[iOS]` 접두사가 붙은 네이티브 로그
- `📝 [JS Console]` 접두사가 붙은 JavaScript 콘솔 로그
- 토큰 저장/조회 상태
- WebView 네비게이션 에러

### Android 콘솔

Android Studio Logcat에서 확인:
- `[Android]` 접두사가 붙은 네이티브 로그
- `📝 [JS Console]` 접두사가 붙은 JavaScript 콘솔 로그
- 토큰 저장/조회 상태
- WebView 네비게이션 에러

### WebView 디버깅

**iOS 시뮬레이터**:
1. Safari → Develop → Simulator → [Your App]
2. WebView 콘솔 확인 가능
3. JavaScript 디버거 사용 가능

**Android 에뮬레이터/기기**:
1. Chrome에서 `chrome://inspect` 접속
2. 디바이스 목록에서 WebView 선택
3. JavaScript 디버거 사용 가능

### SDK 초기화 확인

인증 페이지에서 SDK 초기화 상태를 실시간으로 확인할 수 있습니다:
- SDK 로드 상태
- Project ID 확인
- Script URL 확인

## 문제 해결

### 1. "Native bridge not found" 에러

- WebView가 제대로 로드되지 않았을 수 있습니다
- `AuthWebViewController.swift`의 `setupWebView()` 확인
- JS Bridge 이름이 `nativeBridge`인지 확인

### 2. iOS 빌드 에러

- `Info.plist`의 `API_URL`이 올바르게 설정되어 있는지 확인
- `API_URL` 변경 후 앱을 다시 빌드해야 변경사항이 적용됩니다
- `Info.plist`의 `NSAppTransportSecurity` 설정 확인

### 3. SDK 로드 실패

- `.env.local`의 Project ID 확인
- 네트워크 연결 확인
- Transcodes Dashboard에서 Project ID 확인

### 4. 토큰 저장 실패

**iOS (Keychain)**:
- iOS 시뮬레이터 재시작
- Keychain 접근 권한 확인
- `KeychainHelper.swift`의 에러 로그 확인
- Xcode 콘솔에서 `[iOS] ✅ Verified: Token retrieved from Keychain` 메시지 확인

**Android (EncryptedSharedPreferences)**:
- Android Studio Logcat에서 `[Android]` 로그 확인
- `TokenHelper.kt`의 에러 로그 확인
- 앱 재설치 (SharedPreferences 초기화)

### 5. SDK 초기화 실패

- `.env.local`의 Project ID 확인
- 브라우저 콘솔에서 `window.transcodes` 객체 확인
- `layout.tsx`에서 스크립트 URL이 올바르게 생성되는지 확인
- 네트워크 연결 확인 (CloudFront CDN 접근 가능 여부)

### 6. 토큰이 비어있음

- 인증 성공 후 `getAccessToken()` API가 준비될 때까지 대기
- Private key가 준비되었는지 확인 (`hasPrivateKey()`)
- iOS: Xcode 콘솔에서 토큰 길이 확인
- Android: Logcat에서 토큰 길이 확인

### 7. Android 빌드 에러

- `gradle.properties`의 `API_URL`이 올바르게 설정되어 있는지 확인
- `API_URL` 변경 후 **Gradle Sync**를 다시 실행해야 합니다
- `BuildConfig.API_URL`이 올바르게 생성되었는지 확인 (빌드 후 확인 가능)

### 8. iOS 빌드 에러

- `Info.plist`의 `API_URL`이 올바르게 설정되어 있는지 확인
- `API_URL` 변경 후 앱을 다시 빌드해야 변경사항이 적용됩니다
- Xcode에서 `Info.plist` 파일이 올바르게 인식되는지 확인

## 주요 기능

### ✅ 구현 완료

1. **인증 플로우**: Passkey 기반 인증 완전 구현 (iOS + Android)
2. **토큰 관리**: 
   - iOS: Keychain을 통한 안전한 토큰 저장
   - Android: EncryptedSharedPreferences를 통한 안전한 토큰 저장
3. **Console Logging**: JavaScript 콘솔 로그를 네이티브 콘솔로 전달 (iOS + Android)
5. **Logout**: 토큰 삭제 및 인증 상태 초기화
6. **에러 핸들링**: 상세한 에러 메시지 및 상태 표시
7. **Android 구현**: Android WebView 버전 완전 구현

### 🔄 다음 단계

1. **토큰 검증**: 백엔드 API와 통합하여 토큰 검증
2. **자동 토큰 갱신**: 토큰 만료 전 자동 갱신 로직
3. **에러 재시도**: 네트워크 에러 시 자동 재시도 로직
4. **Biometric 인증**: Face ID / Touch ID / Fingerprint 통합

## 참고 자료

- [Transcodes Documentation](https://transcodes.dev/docs)
- [WKWebView Guide](https://developer.apple.com/documentation/webkit/wkwebview)
- [Keychain Services](https://developer.apple.com/documentation/security/keychain_services)
- [Android WebView Guide](https://developer.android.com/reference/android/webkit/WebView)
- [EncryptedSharedPreferences](https://developer.android.com/topic/security/data)

